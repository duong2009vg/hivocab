 ...
 * Chuẩn giao diện thi tốt nghiệp THPT trên máy tính của Bộ GD&ĐT
 */
(function(window) {
    'use strict';
    ...
    const ThptExam = {
        ...
        // ==========================================
        // 2. RENDER THƯ VIỆN ĐỀ THI (#page-exercises)
        // ==========================================
        renderExamsList() {
            ...
            this.exams.forEach((exam, idx) => {
                ...
                html += `
                <div class="bg-surface-container-lowest rounded-2xl p-5 border border-outline-variant/30 soft-shadow flex flex-col justify-between gap-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
                    <div class="space-y-2.5">
                        <div class="flex items-center justify-between gap-2">
                            <span class="text-[11px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded-md bg-primary/10 text-primary">
                                ĐỀ SỐ ${idx + 1}
                            </span>
                            ${bestBadge}
                        </div>

                        <h3 class="font-bold text-base text-on-surface line-clamp-2 leading-snug" title="${this.escAttr(exam.title)}">
                            ${this.escHtml(exam.title)}
                        </h3>

                        <div class="flex flex-wrap items-center gap-3 text-xs text-on-surface-variant pt-1">
                            ...
                            <span class="flex items-center gap-1 text-emerald-600 font-medium">
                                ...
                                <span>Có giải chi tiết</span>
                            </span>
                        </div>
                    </div>

                    <div class="pt-3 border-t border-outline-variant/20 flex items-center gap-2">
                        <button onclick="window.ThptExam.startExam('${exam.id}', 50)" 
                                class="flex-1 py-2.5 rounded-xl bg-primary hover:bg-primary-dark active:scale-[0.98] text-white font-bold text-xs flex items-center justify-center gap-1.5 shadow-sm transition-all cursor-pointer">
                            <span class="material-symbols-outlined text-[16px]">play_circle</span>
                            <span>Bắt Đầu Thi (50p)</span>
                        </button>
                        <button onclick="window.ThptExam.openCustomTimeModal('${exam.id}')" 
                                class="p-2.5 rounded-xl border border-outline-variant/40 hover:bg-surface-container-high text-on-surface-variant text-xs font-bold transition-colors cursor-pointer" 
        ...
        renderPalette() {
            ...
            for (let i = 1; i <= this.currentExam.total_questions; i++) {
                ...
                let cls = 'w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center font-bold text-xs transition-all relative cursor-pointer select-none shrink-0 ';
        ...
        renderSolutionBox(q) {
            ...
            return `
            <div class="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2 mt-3 text-xs fade-in">
        ...
        enterReviewMode() {
            ...
            const nameEl = document.getElementById('exam-candidate-name');
            if (nameEl) nameEl.textContent = 'CHẾ ĐỘ XEM LỜI GIẢI CHI TIẾT';
